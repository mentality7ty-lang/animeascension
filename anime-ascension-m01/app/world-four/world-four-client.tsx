...
export default function WorldFourClient({userId,character}:{userId:string;character:Character}){
  ...
  const already=character.rank.toLowerCase().includes('spirit sensed');
  const [phase,setPhase]=useState<Phase>(already?'awakened':'gate');
  const [resonance,setResonance]=useState(0);
  const [busy,setBusy]=useState(false);
  const [message,setMessage]=useState(already?'SPIRITUAL PRESSURE · DETECTED':'ROUTING KEY · 100%');

  async function enter(){
    setBusy(true);
    setMessage('CUSTODIAN ROUTE · OPENING');
    await db.from('characters').update({world:'spirit'}).eq('user_id',userId);
    setTimeout(()=>{setPhase('transit');setBusy(false);},500);
  }

  function stabilize(){
    if(busy)return;
    setBusy(true);
    setMessage('FOUR ENERGY SYSTEMS · INTERFERENCE');
    setTimeout(()=>{setPhase('arrival');setBusy(false);},700);
  }

  async function sense(){
    if(busy)return;
    const next=Math.min(100,resonance+20);
    setResonance(next);
    setMessage(next<100?'UNKNOWN PRESSURE · CLOSING IN':'PRESSURE SOURCE · EVERYWHERE');
    if(next<100)return;
    setBusy(true);
    const {error}=await db.from('characters').update({
      power:character.power+60,
      focus:character.focus+3,
      endurance:character.endurance+2,
      rank:`${character.rank} · Spirit Sensed`,
      world:'spirit',
      updated_at:new Date().toISOString()
    }).eq('user_id',userId);
    setBusy(false);
    if(error){setMessage('SAVE FAILED · TRY AGAIN');return;}
    setPhase('awakened');
  }

  return <main className="w4-shell">
    <div className="w4-moon"/><div className="w4-city"/><div className="w4-ash"/>
    <header><span>WORLD 04 // ROUTE AUTHORIZED</span><strong>{message}</strong></header>
    ...
    {phase==='awakened'&&<section className="w4-card result"><span>NEW ENERGY DISCOVERED</span><h1>SPIRITUAL<br/>PRESSURE</h1><p>You cannot control it yet. You can only perceive it — and now that you can, the city is no longer empty.</p><strong>+60 POWER · +3 FOCUS · +2 ENDURANCE</strong><div className="w4-energy"><b>CHAKRA · ACTIVE</b><b>CURSED ENERGY · ACTIVE</b><b>LIFE AURA · ACTIVE</b><b className="unknown">SPIRITUAL PRESSURE · SENSED</b></div><blockquote>UNKNOWN VOICE // “You crossed four worlds and still think the gates are for travel?”</blockquote><button disabled>FIRST SPIRIT ENCOUNTER · NEXT MILESTONE</button><Link href="/life-frontier">RETURN TO LIFE FRONTIER →</Link></section>}
  </main>;
}
